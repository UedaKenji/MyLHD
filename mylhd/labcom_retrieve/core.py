...
class LHDData:
    ...
    def to_pandas(self) -> pd.DataFrame:
        ...
        return pd.DataFrame({"time": self.time, "data": self.data})
...
class LHDRetriever:
    ...
    def _run_retrieve(
        ...
        try:
            ...
            if result.returncode != 0:
                raise RuntimeError(
                    f"Retrieve.exe failed: {result.stderr}\n"
                    + f"Command: {' '.join(cmd)}\n"
    ...
    def retrieve_multiple_channels(
        self,
        diag_name: str,
        shot: int,
        subshot: int = 1,
