...
rcparam_journal = {
    # --- 図全体の設定 ---
    "figure.figsize": (3.0, 2.5),  # 図のサイズ（インチ単位）
...
cm_data_B = [
    [0.0931763, 0.11117333, 0.26151239],
...
def imshow_cbar(ax: plt.Axes, im0: np.ndarray, cbar_title: str | None = None, **kwargs):
    im = ax.imshow(im0, **kwargs)
    divider = mpl_toolkits.axes_grid1.make_axes_locatable(ax)
    cax = divider.append_axes("right", size="5%", pad="3%")
    cbar = plt.colorbar(im, cax=cax, orientation="vertical")
    if cbar_title is not None:
        cbar.set_title(cbar_title)
    ax.set_aspect("equal")


def contourf_cbar(ax: plt.Axes, im0: np.ndarray, cbar_title: str | None = None, **kwargs):
...
def imshow_cbar_bottom(ax: plt.Axes, im0: np.ndarray, cbar_title=None, **kwargs):
    im = ax.imshow(im0, **kwargs)
    divider = mpl_toolkits.axes_grid1.make_axes_locatable(ax)
...
def scatter_cbar(ax: plt.Axes, x, y, c, cbar_title=None, **kwargs):
...
def cmap_line(ax: plt.Axes, x, y, C, cmap="viridis", cbar_title=None, **kwargs):
