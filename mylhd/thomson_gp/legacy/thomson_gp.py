...
def axs_cbar(ax: plt.Axes, im, cbar_title=None, **kwargs):
...
def ShowAnadataInfo(anadata):
    ...
    for i in range(anadata.getDimNo()):
        print(
            "DimName:",
            i,
            anadata.getDimName(i),
            anadata.getDimUnit(i),
            anadata.getDimSize(i),
        )
...
class ThomsonGP:
    ...
    def set_kernel(
        ...
        Ktt_low = LowRankPDM(
            self.Kernel(self.time_inp, self.time_inp, len=time_scale),
            cutoff=time_cutoff,
            iprint=False,
        )
        Krr_low = LowRankPDM(
            self.Kernel(self.rho_vac, self.rho_vac, len=reff_scale),
            cutoff=reff_cutoff,
            iprint=False,
        )
    ...
    def localPosteriorDr(self, timel: np.ndarray, reffl: np.ndarray, parameter: str):
    ...
    def localPosteriorDt(self, timel: np.ndarray, reffl: np.ndarray, parameter: str = "Te"):
...
def plotGP(
    ...
    **sample_kwargs,
    ...
    ax.plot(x_inp, f_inp - sig, color=color, linestyle="dashed", lw=1.0)
...
def plotLogGP(
    ...
    **sample_kwargs,
    ...
    ax.plot(x_inp, np.exp(f_inp - sig), color=color, linestyle="dashed", lw=1.0)
