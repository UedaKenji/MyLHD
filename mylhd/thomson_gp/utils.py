...
class ThomsonGPUtils:
    ...
    def export_nTprofile(self:ThomsonGPCore, dir_path: str, Zeff:np.ndarray|float=1,N_points:int=101) -> None:
        ...
        for time_i in range(self.time_inp.size):
            ...
            Te = self.local_post_mean_function(ValName='Te',function_kind='f',rho_vac_l=f(x),timel=[self.time_inp[time_i]])
            Te = Te[0,:]

            Te = 0.5*(np.exp(Te) + np.exp(Te[::-1]))
            Ne = self.local_post_mean_function(ValName='Ne',function_kind='f',rho_vac_l=f(x),timel=[self.time_inp[time_i]])
            Ne = Ne[0,:]
            ...
            write_profile(f"{dir_path}/nT_profile_{self.shotNo}t{t:.3f}.txt", rho_a99, Ne, Te, zeff)
