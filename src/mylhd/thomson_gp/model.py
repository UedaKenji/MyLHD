...
class Phase(ThomsonGP):
    ...
    @classmethod
    def generate_for_MultiPhase(
        cls, base_obj: ThomsonGPCore, time_start: float, time_end: float, time_independence: bool = False
    ) -> Phase:
    ...
    def set_time(self, iprint=True):
        ...
        self.sigma_ne = self.sigma_ne[
            self.itime_start - self.start_indx_base : self.itime_end - self.start_indx_base, :
        ]
        self.sigma_Te = self.sigma_Te[
            self.itime_start - self.start_indx_base : self.itime_end - self.start_indx_base, :
        ]
...
class ThomsonGPMultiPhase:
    ...
    def plot_im_ax(
        ...
        ax_kwargs = dict(ValName=ValName, funckind=funckind, x_axis=x_axis, ax=ax, vmin=vmin, vmax=vmax, cmap=cmap)
    ...
    def plotProfile(
        ...
        for phase in self.Phases:
            if phase.time_start < time <= phase.time_end:
                phase.plotProfile(
                    ValName=ValName, time=time, ax=ax, sampling=sampling, add_noise=add_noise, color=color
                )
                break
        return fig, ax
